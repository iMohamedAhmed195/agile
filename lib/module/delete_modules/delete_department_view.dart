...
class DeleteDepartView extends StatelessWidget {
  const DeleteDepartView({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => DeleteCubit()..getDepart(),
      child: BlocConsumer<DeleteCubit, DeleteState>(
        listener: (context, state) {

          if (state is DeleteDepartSuccessState) {
            showToast(text: LoggingInterceptor.SuccessMessage, state: ToastState.SUCCESS);
            GoRouter.of(context).push(AppRouter.kHomeAdmin);
          }else if (state is DeleteDepartErrorState) {
                  ...
                  child: Column(
                    children: [
                      const TextSecLogin(
                          ...
                          para:
                          ' Delete department to decrease some user from here.'),
                      Padding(
                        ...
                        child: Container(
                          ...
                          child: DropdownButton(
                            ...
                            isExpanded: true,
                            value: DeleteCubit.get(context).departChoose,
                            onChanged: (newValue) {
                              DeleteCubit.get(context).chooseDepart(newValue!);
                            },
                            items: DeleteCubit.get(context).departId.map((valueItem) {
                          ...
                          Container(
                            ...
                            child: MaterialButton(
                              onPressed: () {
                                ...
                                DeleteCubit.get(context).deleteDeparts(departId: DeleteCubit.get(context).departChoose);
                              },
                              child: const Text(
                                'Delete Depart',
